...
Public Class frmproductinquiry
    ...
    Sub loadinventory()
        ...
        Try
            ...
            End If
            con.Open()
            cmd = New SqlCommand("select * from tblInventory as iv inner join tblproduct as p on iv.id = p.id inner join tblbrand as b on p.bid = b.brandID inner join tblcategory as c on p.cid =	c.catID inner join tblformulations as f on p.fid = f.formID inner join tbldosage as d on p.did = d.dosageID inner join tblgeneric as g on p.gid = g.genericID  inner join tblunit as un on p.uid = un.unitID ", con)
            dr = cmd.ExecuteReader
            ...
            While dr.Read
                ...
                Guna2DataGridView1.Rows.Add(dr("InventoryID").ToString(), i, dr("id").ToString(), img, dr("barcode").ToString(), itemDescription, dr("Quantity").ToString(), dr("price").ToString())
            End While
