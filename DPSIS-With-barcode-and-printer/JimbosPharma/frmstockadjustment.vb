Public Class frmstockadjustment
    ...
    Sub stockrecord()  '   READING ATTRIBUTES FROM TABLE PRODUCT WITH USING INNER JOIN
        ...
        cmd = New SqlClient.SqlCommand("select * from tblInventory as iv  inner join tblproduct as p on iv.id = p.id inner join tblbrand as b on p.bid = b.brandID inner join tblcategory as c on p.cid = c.catID inner join tblformulations as f on p.fid = f.formID inner join tbldosage as d on p.did = d.dosageID inner join tblgeneric as g on p.gid = g.genericID  inner join tblunit as un on p.uid = un.unitID   where " & cbofilter.Text & " like '" & txtsearch.Text & "%'", con)
    ...
    Private Sub Guna2DataGridView2_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles Guna2DataGridView2.CellContentClick
        Try
            ...
            If colname = "colselect" Then
                con.Open()
                cmd = New SqlClient.SqlCommand("select * from tblInventory as iv  inner join tblproduct as p on iv.id = p.id inner join tblbrand as b on p.bid = b.brandID inner join tblcategory as c on p.cid =	c.catID inner join tblformulations as f on p.fid = f.formID inner join tbldosage as d on p.did = d.dosageID inner join tblgeneric as g on p.gid = g.genericID  inner join tblunit as un on p.uid = un.unitID where InventoryID like '" & Guna2DataGridView2.Rows(e.RowIndex).Cells(1).Value.ToString & "'", con)
                dr = cmd.ExecuteReader
                dr.Read()
                If dr.HasRows Then
